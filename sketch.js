...
function draw() {
  ...
  if (gameState === 1) {
    ...
    //VISBLING THE THINGS IN THE GAME
    ...
    changedHouse.visible = true;
    palaceSprite.visible = false;
    youwin.visible = false;
    gameOver_sprite.visible = false;

    for (var x = 0; x < 93; x++) {
      Hfence[x].visible = true;
    }
    ...
    if (player.isTouching(changedHouse)) {
      queen.visible = true;
      queen.x = player.x;
      queen.y = player.y;
    }

    player.collide(Hfence);
    player.collide(house);

    player.collide(changedHouse);

    camera.x = player.x;
    camera.y = player.y;

    Hfence.depth = queen.depth;
    queen.depth = queen.depth + 1;
    ...
    if (keyCode === 70 && queen.isTouching(player)) {
      gameState = 2;
    }

    if (ghost1.isTouching(player) ||
      ghost2.isTouching(player) ||
      ghost3.isTouching(player) ||
      ghost4.isTouching(player) ||
      ghost5.isTouching(player) ||
      ghost6.isTouching(player) ||
      ghost7.isTouching(player) ||
      score < 0) {
      gameState = 4;
    }

    setTimeout(ghost1Move, 3000);
    stand();
    playerMovement();
  }
  ...
  if (gameState === 2) {
    ...
    palaceSprite.visible = true;
    youwin.visible = false;
    gameOver_sprite.visible = false;

    //THEY ARE THE LOGICS IN THE GAME********************
    ...
    if (player.isTouching(palaceSprite)) {
      gameState = 3
      console.log('this')
    }
  ...
  if (gameState === 3) {
    ...
    //VISIBLING THE THINGS THAT WE ARE NEED*************
    ...
    //WE ARE GIVING LOGIC TO THE GAME*********
  ...
  if (gameState === 4) {
    ...
    //THEY ARE THE LOGICS IN OUR CODE **********
    ...
    gameOver_sprite.x = player.x;
    gameOver_sprite.y = player.y;
...
function randomSketch() {
  ...
  randomPos = Math.round(random(1, 20))
  switch (randomPos) {
    case 1: queen.x = a.x; queen.y = a.y; a.visible = false;
      break;
    case 2: queen.x = b.x; queen.y = b.y; b.visible = false;
      break;
    case 3: queen.x = c.x; queen.y = c.y; c.visible = false;
      break;
    case 4: queen.x = d.x; queen.y = d.y; d.visible = false;
      break;
    case 5: queen.x = e.x; queen.y = e.y; e.visible = false;
      break;
    case 6: queen.x = f.x; queen.y = f.y; f.visible = false;
      break;
    case 7: queen.x = g.x; queen.y = g.y; g.visible = false;
      break;
    case 8: queen.x = h.x; queen.y = h.y; h.visible = false;
      break;
    case 9: queen.x = i.x; queen.y = i.y; i.visible = false;
      break;
    case 10: queen.x = j.x; queen.y = j.y; k.visible = false;
      break;
    case 11: queen.x = k.x; queen.y = k.y; l.visible = false;
      break;
    case 12: queen.x = l.x; queen.y = l.y; m.visible = false;
      break;
    case 13: queen.x = n.x; queen.y = n.y; n.visible = false;
      break;
    case 14: queen.x = o.x; queen.y = o.y; o.visible = false;
      break;
    case 15: queen.x = p.x; queen.y = p.y; p.visible = false;
      break;
    case 16: queen.x = q.x; queen.y = q.y; q.visible = false;
      break;
    case 17: queen.x = r.x; queen.y = r.y; r.visible = false;
      break;
    case 18: queen.x = s.x; queen.y = s.y; s.visible = false;
      break;
    case 19: queen.x = t.x; queen.y = t.y; t.visible = false;
      break;
    case 20: queen.x = m.x; queen.y = m.y; m.visible = false;
      break;
  }
}
function startSound() {
...
function instruction() {
  if(!instructions2.isPlaying()){
    instructions2.play();
    instructions2.setVolume(1);
    instructionButton.html("Pause Instructions")
  }else{
    instructions2.stop();
    instructionButton.html("Play Instructions")
  }
